...
> []SharedFolder CreateSharedFolder(ctx, id).SharedFolder(sharedFolder).VmPassword(vmPassword).Execute()
...
func main() {
	...
	sharedFolder := *vmrest.NewSharedFolder("FolderId_example", "HostPath_example", int32(123)) // SharedFolder | Parameters of the shared folder to mount
	...
	resp, r, err := apiClient.VMSharedFoldersManagementAPI.CreateSharedFolder(context.Background(), id).SharedFolder(sharedFolder).VmPassword(vmPassword).Execute()
	if err != nil {
 ...
 **sharedFolder** | [**SharedFolder**](SharedFolder.md) | Parameters of the shared folder to mount | 
...
> []SharedFolder UpdataSharedFolder(ctx, id, folderId).SharedFolderParameter(sharedFolderParameter).VmPassword(vmPassword).Execute()
...
func main() {
	...
	sharedFolderParameter := *vmrest.NewSharedFolderParameter("HostPath_example", int32(123)) // SharedFolderParameter | Parameters of the shared folder to update to
	vmPassword := "vmPassword_example" // string | VM password (optional)

	configuration := vmrest.NewConfiguration()
	apiClient := vmrest.NewAPIClient(configuration)
	resp, r, err := apiClient.VMSharedFoldersManagementAPI.UpdataSharedFolder(context.Background(), id, folderId).SharedFolderParameter(sharedFolderParameter).VmPassword(vmPassword).Execute()
	if err != nil {
 ...
 **sharedFolderParameter** | [**SharedFolderParameter**](SharedFolderParameter.md) | Parameters of the shared folder to update to | 
 **vmPassword** | **string** | VM password | 
